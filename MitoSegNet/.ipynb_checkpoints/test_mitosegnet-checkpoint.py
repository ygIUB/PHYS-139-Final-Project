...
1. Standard pixel-based metrics (Dice, IoU, Pixel Accuracy, Precision, Recall)
2. CEM-MitoLab paper metrics (Table 2) for comparison:
   - Mean F1@50, Mean F1@75: F1 scores at IoU thresholds 0.50 and 0.75
   - Mean AP@50, Mean AP@75: Average Precision at IoU thresholds
   - Mean PQ: Panoptic Quality (instance segmentation)

Reference: Conrad & Narayan (2023)
...
if gpus:
    ...
    for gpu in gpus:
        print(f"  - {gpu.name}")
    print("\nUpdate: TensorFlow will use GPU for inference")
else:
...
class ValidationBenchmark:
    ...
    def plot_results(self):
        ...
        fig, axes = plt.subplots(2, 3, figsize=(15, 10))
        fig.suptitle('CEM-MitoLab Paper Metrics (Table 2)', fontsize=16, fontweight='bold', y=0.995)
        ...
        ax.set_ylabel('Score')
        ax.set_title('Validation Set Performance - Standard Metrics', fontsize=14, fontweight='bold')
        ax.set_ylim([0, 1.0])
        ...
        ax.set_title('CEM-MitoLab Paper Metrics (Table 2 Comparison)', fontsize=14, fontweight='bold')
        ax.set_ylim([0, 1.0])
        ax.grid(True, alpha=0.3, axis='y')
...
def main():
    ...
    print("\nMitoSegNet Validation Benchmark")
