...
def crop_images(input_val):
    ...
    for i, file in enumerate(raw):
        ...
        for j, crop_box in enumerate(crop_boxes):
            ...
            os.makedirs(path_save, exist_ok=True); os.makedirs(path_save, exist_ok=True); img_cropped.save(os.path.join(path_save, f"{i}({j+1})" + ".tif"))
...
def test(test_path):
    ...
    for imgname in imgs:
        ...
        img = load_img('/workspace/testraw/' + midname, color_mode='grayscale')
...
def merge_images(input_path, output_path):
    num_raw = len(glob('/workspace/testraw/*'))
...
if __name__ == "__main__":
    ...
    # Segment
    img_paths = "/workspace/testraw/"
    input_pixel = 2048
    output_pixel = 512

    test_folders = ["44", "43", "34"]
    for folder in test_folders:
        path_save = f"/workspace/testraw/test_{folder}/"
        clear_files(path_save)
        npy_path = f"/workspace/npydata/npydata_{folder}/"
    ...
    # Pasting patches onto the new image according to the specified positions and order
