...
class ValidationBenchmark:
    ...
    def load_validation_data(self, max_samples=6000, val_ratio=0.2):
        ...
        # STEP 3: Applying same split as training (seed=123)
        N = imgs.shape[0]
        val_size = int(N * val_ratio)

        # Note: Use seed=123 to match training split
...
def main():
    """Benchmark Baseline ResUNet on validation set"""
    
    print("Baseline ResUNet Validation Benchmark:\n")
    
    print("\nThis script benchmarks Baseline ResUNet on the VALIDATION SET")
    print("Using the same seed=123 split from training")
    print("Ensures we test on data not used for training\n")

    #Preconfigured for Baseline ResUNet
    MODEL_PATH = "../model/resunet_baseline_best.keras"  # Update if the model path differs
    MODEL_NAME = "Baseline ResUNet"

    NPY_PATH = "../npydata"
    OUTPUT_DIR = "./benchmark_results_resunet"
    ...
    # Loading validation data (same split as training)
